...
import com.lauin.store.dtos.ProductSummaryDTO;
...
public interface ProductRepository extends JpaRepository<Product, Long>, ProductCriteriaRepository, JpaSpecificationExecutor<Product> {
    ...
    @Query("select new com.lauin.store.dtos.ProductSummaryDTO(p.id, p.name) from Product p where p.category = :category")
